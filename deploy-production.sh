...
if [ -f .env.production ]; then
  ...
  source .env.production
    ...
    echo ".env.production file was not found. The deployment script requires project variables to be defined."
    echo "Create .env.production in the project root with the following variables: "
    echo ""
    echo "GCP_PROJECT=<your gcp project>"
    echo "GCP_REGION=<gcp region to deploy function to>"
    echo "FUNCTION_NAME=<name of gcp function>"
    exit 1
fi
